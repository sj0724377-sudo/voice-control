...
\* \*\*`web\_server.py`\*\*: \*\*\[PRIMARY SYSTEM ENGINE]\*\* Run this script to initialize the local Flask interface, establish ultra-low-latency WebSocket channels with the ESP32 chassis gateway, process incoming camera matrices on your dedicated GPU, and process voice inputs via local ASR.
...
\* \*\*Error Isolation:\*\* The system establishes the camera center at pixel column \*\*320\*\* (`FRAME\_CENTER = 640 / 2`). The controller constantly tracks the horizontal center point of the object's bounding box and computes the exact pixel deviation (delta\_x):



$$\\text{delta\\\_x} = \\frac{x\_1 + x\_2}{2} - 320$$





\* \*\*Proportional Vectoring:\*\* This error distance is multiplied by a proportional steering gain (`kp = 0.15`) to scale the steering response:



$$\\text{steering\\\_bias} = \\text{int}(\\text{delta\\\_x} \\times \\text{kp})$$





\* \*\*Differential Speed Mapping:\*\* The bias values adjust the independent left and right wheel speeds relative to a baseline forward velocity:



$$\\text{Left Motor Speed} = \\text{base\\\_speed} + \\text{steering\\\_bias}$$





$$\\text{Right Motor Speed} = \\text{base\\\_speed} - \\text{steering\\\_bias}$$
